...
bool var1 = true;              //	1 byte	true or false	    Digital states (ON/OFF), flags.
...
char var3 = -127;              //	1 byte	-128 to 127	        Single characters (e.g., 'A').
...
unsigned long var 7 = 4567890;    //  4 bytes	0 to 4.2B	    Time tracking (essential for millis()).
// розширені додатні числа, займають в памяті тих же 4 байта, але тільки додатні від 0 до 4.2 мільярда
float var 8 = 1.234567;	        //  4 bytes	~3.4E±38	        Numbers with decimals (accurate to ~6-7 digits).
// число з плаваючою крапкою, або дійсне, або число з дробовою частиною. Наприклад: 4.12345. Займає 4 байта в памяті
double var 9 = 1.234567; 	        //  4/8 bytes	Same as float	On many Arduinos, double is identical to float.
